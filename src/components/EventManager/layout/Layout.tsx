      ...
      <div className="flex h-screen w-screen bg-gradient-to-br from-[#0f0c1a] to-[#1c1429] text-white overflow-hidden">
        ...
        <aside className="w-72 bg-gradient-to-br from-[#1e1b2e] to-[#2c2a40] shadow-2xl flex flex-col">
          ...
          <nav className="flex-1 px-4 py-6 overflow-y-auto space-y-6">
            <div>
              <NavItem href="/event-manager" icon={FaHome} isActive={isActiveRoute('/event-manager')}>
                Dashboard
              </NavItem>
            </div>
            ...
            <div>
              ...
              {expandedSections.events && (
                <div className="ml-4 space-y-1 mt-2">
                  <NavItem href="/event-manager/my-events" icon={FaCalendarAlt} isActive={isActiveRoute('/event-manager/my-events')}>
                    Sự kiện của tôi
                  </NavItem>
                  <NavItem href="/event-manager/create-event" icon={FaPlus} isActive={isActiveRoute('/event-manager/create-event')}>
                    Tạo sự kiện mới
                  </NavItem>
                  
                  <NavItem href="/event-manager/pending-events" icon={FaClock} isActive={isActiveRoute('/event-manager/pending-events')}>
                    Sự kiện đang chờ duyệt
                  </NavItem>
                  <NavItem href="/event-manager/approved-events" icon={FaCheckCircle} isActive={isActiveRoute('/event-manager/approved-events')}>
                    Sự kiện đã duyệt
                  </NavItem>
                  <NavItem href="/event-manager/collaborators" icon={FaUsers} isActive={isActiveRoute('/event-manager/collaborators')}>
            ...
            <div>
              ...
              {expandedSections.tickets && (
                <div className="ml-4 space-y-1 mt-2">
                  ...
                  <NavItem href="/event-manager/tickets/manage" icon={FaTicketAlt} isActive={isActiveRoute('/event-manager/tickets/manage')}>
                    Quản lý vé
                  </NavItem>
                  <NavItem href="/event-manager/discount-codes" icon={FaPercent} isActive={isActiveRoute('/event-manager/discount-codes')}>
                    Mã giảm giá
                  </NavItem>
                  <NavItem href="/event-manager/ticket-sales" icon={FaChartBar} isActive={isActiveRoute('/event-manager/ticket-sales')}>
                    Theo dõi bán vé
                  </NavItem>
                  <NavItem href="/event-manager/check-ins" icon={FaCheckCircle} isActive={isActiveRoute('/event-manager/check-ins')}>
                    Check-in & QR Code
                  </NavItem>
                </div>
              )}
            ...
            <div>
              ...
              {expandedSections.analytics && (
                <div className="ml-4 space-y-1 mt-2">
                  <NavItem href="/event-manager/analytics/overview" icon={FaChartBar} isActive={isActiveRoute('/event-manager/analytics/overview')}>
                    Tổng quan
                  </NavItem>
                  
                  <NavItem href="/event-manager/analytics/participants" icon={FaUsers} isActive={isActiveRoute('/event-manager/analytics/participants')}>
                    Danh sách người tham gia
                  </NavItem>
                  <NavItem href="/event-manager/reviews" icon={FaEye} isActive={isActiveRoute('/event-manager/reviews')}>
                    Đánh giá sự kiện
                  </NavItem>
                  <NavItem href="/event-manager/analytics/predictions" icon={FaChartBar} isActive={isActiveRoute('/event-manager/analytics/predictions')}>
                    Dự đoán AI
                  </NavItem>
                </div>
              )}
            ...
            <div>
              ...
              {expandedSections.content && (
                <div className="ml-4 space-y-1 mt-2">
                  <NavItem href="/event-manager/news" icon={FaNewspaper} isActive={isActiveRoute('/event-manager/news')}>
                    Quản lý tin tức
                  </NavItem>
                  <NavItem href="/event-manager/notifications" icon={FaBell} isActive={isActiveRoute('/event-manager/notifications')}>
                    Thông báo
                  </NavItem>
                  <NavItem href="/event-manager/chat" icon={FaComments} isActive={isActiveRoute('/event-manager/chat')}>
                    Chat hỗ trợ
                  </NavItem>
                </div>
              )}
