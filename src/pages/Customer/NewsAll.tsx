...
const NewsCardSkeleton = () => (
  <div className="animate-pulse rounded-2xl bg-white dark:bg-gray-800 h-[400px]"></div>
);
...
const NewsAll: React.FC = () => {
  ...
  useEffect(() => {
    const loadNews = async () => {
      setLoading(true);
      try {
        const { news, totalCount } = await searchNewsAPI({
          ...debouncedFilters,
          page: currentPage,
          pageSize: PAGE_SIZE,
        });
  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow">
        <h3 className="font-bold text-lg text-gray-900 dark:text-white line-clamp-2 mb-2 group-hover:text-blue-600 dark:group-hover:text-purple-400 transition-colors">
          {news.newsTitle}
        </h3>
        <p className="text-gray-600 dark:text-gray-400 line-clamp-3 text-sm flex-grow">
