...
export const CanceledEventList = ({
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        console.log('🔄 Loading categories from canceled events...');
        // Load initial events to extract categories
        const response = await getCanceledEventsWithFilter({ page: 1, pageSize: 100 });
        console.log('📥 Canceled events API response for categories:', response);
        if (response && response.data && response.data.items) {
          ...
          console.log('📋 Extracted categories from canceled events:', categoryNames);
          setAllCategories(categoryNames);
        } else {
          console.warn('⚠️ No canceled events found to extract categories:', response);
          setAllCategories([]);
        }
      } catch (error) {
        console.error('❌ Failed to load categories from canceled events:', error);
        setAllCategories([]);
      }
    };
