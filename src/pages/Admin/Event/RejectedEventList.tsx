...
export const RejectedEventList = ({
  ...
  const [rejectedEventSearch, setRejectedEventSearch] = useState('');
  ...
  const searchRef = useRef(rejectedEventSearch);
  ...
  const fetchData = (showLoading = true) => {
    ...
    const paginationParams = {
      page: rejectedEventSearch ? 1 : filters.page,
    ...
    getRejectedEventsWithFilter(params)
      .then((res) => {
        if (res && res.data) {
          setData({
            ...res.data,
      ...
      <div className="overflow-x-auto">
        <div className={`p-4 ${getEventListCardClass()}`}>
          ...
          <Table className={`${getEventListTableClass()} ${getEventListTableBorderClass()}`}>
            <TableHeader>
              <TableRow
                className={`bg-red-200 hover:bg-red-200 ${getEventListTableHeaderBorderClass()}`}
              >
            ...
            <TableBody className={`${getEventListTableClass()} ${getEventListTableBorderClass()}`}>
              {items.length === 0 ? (
                <>
                  {/* Show "No rejected events found" message */}
                  <TableRow
