...
export const CompletedEventList = ({ onLoadingChange }: CompletedEventListProps) => {
  ...
  useEffect(() => {
    (async () => {
      // Lấy tất cả categoryId duy nhất từ các event completed
  ...
  useEffect(() => {
    ...
    getCompletedEvents({ page, pageSize })
      .then((res) => {
        if (res && res.data) {
          setData({
    ...
    const reload = () =>
      getCompletedEvents({ page, pageSize }).then((res) => {
        if (res && res.data) {
          setData({
            items: res.data.items,
            totalItems: res.data.totalItems,
                            ...
                            <PaginationItem key={i}>
                              <PaginationLink
                                ...
                                className={`transition-colors rounded 
                                  ${
                                    i === page
                                      ? 'bg-blue-500 text-white border hover:bg-blue-600 hover:text-white'
