...
export const EventManagerList = () => {
  ...
  const fetchUsers = async (isSearching = false) => {
    ...
    try {
      ...
      const response = await getEventManagersWithFilter(params);
      if (response.flag) {
        ...
        toast.error(response.message || t('failedToFetchEventManagerUsers'));
      }
    } catch (error) {
      // Handle API error with specific message
      if (error instanceof Error) {
        toast.error(error.message || t('failedToFetchEventManagerUsers'));
      } else {
                  toast.error(t('failedToFetchEventManagerUsers'));
      }
        ...
        <div className={`p-4 ${getAdminListCardClass()}`}>
            ...
            <div className="flex-1 flex items-center gap-2">
              ...
              >
                <input
                  ...
                  onChange={(e) => {
                    setEventManagerSearch(e.target.value);
          ...
          <Table className={`${getAdminListTableClass()} ${getAdminListTableBorderClass()}`}>
            ...
            <TableBody className={`${getAdminListTableClass()} ${getAdminListTableBorderClass()}`}>
              {users.length === 0 ? (
                <>
                  {/* Show "No event managers found" message */}
                  <TableRow
