  ...
  const fetchUsers = async (isSearching = false) => {
    ...
    try {
      ...
      if (response.flag) {
        ...
        toast.error(response.message || t('failedToFetchCollaboratorUsers'));
      }
    } catch (error) {
      // Handle API error with specific message
      if (error instanceof Error) {
        toast.error(error.message || t('failedToFetchCollaboratorUsers'));
      } else {
                  toast.error(t('failedToFetchCollaboratorUsers'));
      }
