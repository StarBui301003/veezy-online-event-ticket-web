...
import { getAllWithdrawalRequests } from '@/services/Admin/fund.service';
...
export const AllWithdrawRequests = ({ onPendingChanged }: { onPendingChanged?: () => void }) => {
  ...
  const refreshData = () => {
    ...
    getAllWithdrawalRequests({ pageNumber: 1, pageSize }) // Reset về trang 1
      .then((res) => {
  ...
  useEffect(() => {
    setLoading(true);
    getAllWithdrawalRequests({ pageNumber: page, pageSize })
      .then((res) => {
        setData(res.data.data);
      })
      .finally(() => setTimeout(() => setLoading(false), 500));
    ...
    // Listen for fund-related events that affect all withdrawal requests
    const reloadData = () => {
      console.log('Fund event received, refreshing all withdrawal requests...');
      refreshData();
    };
    ...
    <div className="p-3">
      ...
      <div className="overflow-x-auto">
        <div className="p-4 bg-white rounded-xl shadow">
          {/* Search + Enable Withdrawal button */}
            ...
            <TableBody>
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map((item, idx) => (
                  <TableRow key={item.transactionId} className="hover:bg-gray-50">
                            ...
                            <PaginationItem key={i}>
                              <PaginationLink
                                ...
                                className={`transition-colors rounded 
                                  ${
                                    i === page
                                      ? 'bg-gray-500 text-white border hover:bg-gray-600 hover:text-white'
