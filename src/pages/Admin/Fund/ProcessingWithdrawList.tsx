...
import { getProcessingWithdrawals, FundFilterParams } from '@/services/Admin/fund.service';
...
const ProcessingWithdrawList = ({ onProcessingChanged }: { onProcessingChanged?: () => void }) => {
  const {
  ...
  const searchRef = useRef(processingSearch);
  ...
  const fetchData = (showLoading: boolean = true) => {
    ...
    getProcessingWithdrawals(params)
      .then(async (res) => {
        if (res && res.data && res.data.data) {
          ...
          if (!processingSearch && amountRange[0] === 0 && amountRange[1] === 1000000) {
  ...
  const refreshData = () => {
    setProcessingSearch(''); // Reset search when refresh
    setFilters((prev) => ({ ...prev, Page: 1 })); // Reset to page 1
    onProcessingChanged?.(); // Call callback to update badge
  };
        ...
        <div className={`p-4 rounded-xl shadow ${getAdminListCardClass()}`}>
          ...
          >
            ...
            >
              {items.length === 0 ? (
                <>
                  {/* Show "No processing withdraw requests found" message */}
                  <TableRow
                  ...
                  >
                    ...
                    >
                      No processing withdraw requests found.
                    </TableCell>
                  </TableRow>
