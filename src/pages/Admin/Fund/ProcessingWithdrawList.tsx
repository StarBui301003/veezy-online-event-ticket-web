...
const ProcessingWithdrawList = ({ onPendingChanged }: { onPendingChanged?: () => void }) => {
          ...
          <Table className="min-w-full">
            ...
            <TableBody className="min-h-[400px]">
              {items.length === 0 ? (
                ...
                <>
                  {items.map((item, idx) => (
                    <TableRow key={item.transactionId} className="hover:bg-blue-50">
                      <TableCell className="text-center">
                        {(filters.Page - 1) * filters.PageSize + idx + 1}
                      </TableCell>
                      <TableCell
                        className="truncate max-w-[180px] overflow-hidden text-ellipsis whitespace-nowrap"
