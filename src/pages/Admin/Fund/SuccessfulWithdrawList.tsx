...
import { getSuccessfulWithdrawals, FundFilterParams } from '@/services/Admin/fund.service';
...
const SuccessfulWithdrawList = ({ onSuccessfulChanged }: { onSuccessfulChanged?: () => void }) => {
  const {
  ...
  const searchRef = useRef(successfulSearch);
  ...
  useEffect(() => {
    searchRef.current = successfulSearch;
  }, [successfulSearch]);
  ...
  const fetchData = (showLoading = true) => {
    ...
    const paginationParams = {
      Page: successfulSearch ? 1 : filters.Page,
    ...
    console.log('🔍 Successful Fund Search Parameters:', {
    ...
    getSuccessfulWithdrawals(params)
      .then(async (res) => {
        if (res && res.data && res.data.data) {
          ...
          if (!successfulSearch && amountRange[0] === 0 && amountRange[1] === 1000000) {
  ...
  const refreshData = () => {
    setSuccessfulSearch(''); // Reset search when refresh
    setFilters((prev) => ({ ...prev, Page: 1 })); // Reset to page 1
    onSuccessfulChanged?.(); // Call callback to update badge
  };
        ...
        <div className={`p-4 rounded-xl shadow ${getAdminListCardClass()}`}>
            ...
            <div className="flex-1 flex items-center gap-2">
              ...
              >
                <input
                  ...
                  value={successfulSearch}
                  onChange={(e) => {
                    setSuccessfulSearch(e.target.value);
          ...
          >
            ...
            >
              {items.length === 0 ? (
                <>
                  {/* Show "No successful withdraw requests found" message */}
                  <TableRow
                  ...
                  >
                    ...
                    >
                      No successful withdraw requests found.
                    </TableCell>
                  </TableRow>
